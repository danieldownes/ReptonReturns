...
public class rr2fire : MonoBehaviour 
{
	...
	void NewTime()
	{
	    fTime = Random.Range(1, 2); //* rrGame.sngGameSpeed;
	}
...
End Function
	...
	void GrowFire()
	{
		...
		int iFires = rr2gameObject.loadedLevel.iPieceTot[102 - 32]; //((int)(char)rr2level.enmPiece.Fungus)
        ...
        n = 0;
		x = 0;
        for( y = 0; y < rr2gameObject.loadedLevel.iMapSizeY && n < iFireToGrow; y++)
		{
			for( x = 0; x < rr2gameObject.loadedLevel.iMapSizeX && n < iFireToGrow; x++)
			{
            	if( rr2gameObject.loadedLevel.RrMapDetail[x,y].TypeID == (char)rr2level.enmPiece.Fungus)
                	n++;				
            }
        }
		...
		//if( x > 0)
		//{
	        cT = (char)rr2gameObject.loadedLevel.RrMapDetail[x-1,y].TypeID;
	        if( cT == 'i' || cT == 'm' || cT == '0')
			{
	            bGrow[0] = true;
	            n++;
		...
		//if( y > 0)
		//{
	        cT = (char)rr2gameObject.loadedLevel.RrMapDetail[x,y-1].TypeID;
			Debug.Log( "UP=" +cT);
	        if( cT == 'i' || cT == 'm' || cT == '0')
			{
	            bGrow[2] = true;
